...
class HiggsAudioServeEngine:
    def __init__(
        ...
        tokenizer_name_or_path: Optional[str] = "bosonai/higgs-audio-v2-generation-3B-base",
        device: str = "cuda",
    ...
    def _prepare_inputs(self, chat_ml_sample: ChatMLSample, force_audio_gen: bool = False, chatml_sample_uncond: Optional[ChatMLSample] = None):
        ...
        if chatml_sample_uncond is not None:
            input_tokens_uncond, _, _, _ = prepare_chatml_sample(chatml_sample_uncond, self.tokenizer)
            input_tokens_uncond.extend(postfix)
            # both conditional and unconditional samples share the same context audio
            # logic here should be verified for general cases
            curr_sample_uncond = ChatMLDatasetSample(
                input_ids=torch.LongTensor(input_tokens_uncond),
                label_ids=None,
                audio_ids_concat=audio_ids_concat,
                audio_ids_start=audio_ids_start,
                audio_waveforms_concat=None,
        ...
        data = self.collator([sample, curr_sample_uncond] if chatml_sample_uncond is not None else [sample])
        inputs = asdict(data)
    ...
    def generate(
        ...
        unconditional_chat_ml_sample: Optional[ChatMLSample] = None,
    ):
        """
        ...
        with torch.no_grad(), self.generate_lock:
            inputs = self._prepare_inputs(chat_ml_sample, force_audio_gen=force_audio_gen, chatml_sample_uncond=unconditional_chat_ml_sample)
