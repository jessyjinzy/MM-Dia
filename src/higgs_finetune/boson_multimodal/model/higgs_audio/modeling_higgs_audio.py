...
class HiggsAudioModel(HiggsAudioPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if use_static_cache:
            ...
            if hidden_states.shape[1] == 1:
                ...
                is_decoding_audio_token = audio_discrete_codes_mask[0].item()
            else:
        ...
        #print("Hidden states before decoder:", hidden_states.shape)
        #print("use_static_cache:", use_static_cache) 
        #print("cache_position:", cache_position)
        #print("past_key_values:", past_key_values)
        #print("use_cache:", use_cache)
        #print("is_decoding_audio_token:", is_decoding_audio_token)
        #print("audio_discrete_codes_mask:", audio_discrete_codes_mask)
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        This method is modified to support Classifier-Free Guidance (CFG).

        If `guidance_scale` > 1.0, it performs CFG, requiring `unconditional_input_ids`.
        Otherwise, it performs standard sampling.
        ...
        """
        guidance_scale = generation_config.generation_kwargs.get("guidance_scale", 1.0)
        guidance_scale_text = generation_config.generation_kwargs.get("guidance_scale_text", 1.0)
        is_cfg_enabled = guidance_scale > 1.0 or guidance_scale_text > 1.0
        
        #if is_cfg_enabled:
        #    assert input_ids.shape[0] == 2, "When using CFG, please make sure to pass in both conditional and unconditional input_ids!"
        #    batch_size = input_ids.shape[0] // 2

        #    # Prepare inputs for CFG if enabled
        #    # ==================================
        #    # Double the inputs to process conditional and unconditional prompts in one pass
        #    #input_ids = torch.cat([input_ids, unconditional_input_ids], dim=0)
        #    # in this implementation, we assume the conditional and unconditional input_ids are concatenated already.

        #    uncond_input_ids = input_ids[batch_size:]
        #    #input_ids = input_ids[:batch_size]

        #    uncond_attention_mask = model_kwargs["attention_mask"][batch_size:]
        #    #model_kwargs["attention_mask"] = model_kwargs["attention_mask"][:batch_size]

        #    uncond_len = uncond_attention_mask.sum().item()
        #    uncond_input_ids = uncond_input_ids[:, :uncond_len]
        #    uncond_attention_mask = uncond_attention_mask[:, :uncond_len]

        #    uncond_model_kwargs = model_kwargs.copy() 
        #    uncond_model_kwargs.update({"input_ids": uncond_input_ids, "attention_mask": uncond_attention_mask})

        #assert input_ids.shape[0] // (2 if is_cfg_enabled else 1) == 1, "Only support batch_size=1 in _sample()"
        ...
        # keep track of which sequences are already finished (only for the conditional part)
        ...
        # Initialize the audio variables based on the input prompt. (conditional only)
        ...
        while self._has_unfinished_sequences(
            ...
            #print(f"Current input_ids: {input_ids}")
            #print(f"Uncond input_ids: {uncond_input_ids}") if is_cfg_enabled else None
            # Determine generation mode based on the conditional input
            ...
            if init_model_input or not generation_config.use_cache:
                ...
                if is_audio_generation_mode and generation_config.use_cache:
                    if is_cfg_enabled:
                        model_inputs["audio_out_ids"] = model_kwargs["audio_out_ids"][:, -1:].repeat(1, 3)
                        #print("Audio out ids for CFG:", model_inputs["audio_out_ids"])
                        model_inputs["audio_out_ids_start"] = torch.tensor([0, 1, 2], dtype=torch.long, device=input_ids.device)
                    else:
            ...
            #uncond_model_kwargs["use_cache"] = False
            #uncond_model_inputs = {"input_ids": uncond_input_ids, **uncond_model_kwargs} if is_cfg_enabled else None
            #print(f"Uncond kwargs: {uncond_model_kwargs}") if is_cfg_enabled else None
            ...
            if is_cfg_enabled:
                # process the text part logits
                cond_logits = outputs.logits[:1] 
                uncond_logits = outputs.logits[1:2]
                cfg_logits = uncond_logits + guidance_scale * (cond_logits - uncond_logits)
                outputs.logits = cfg_logits

                # process the audio part logits
                cond_audio_logits = outputs.audio_logits[:1]
                uncond_audio_logits = outputs.audio_logits[1:2]
                uncond_text_audio_logits = outputs.audio_logits[2:]
                cond_audio_logits = uncond_text_audio_logits + guidance_scale_text * (cond_audio_logits - uncond_text_audio_logits)
                cfg_audio_logits = uncond_audio_logits + guidance_scale * (cond_audio_logits - uncond_audio_logits)
                outputs.audio_logits = cfg_audio_logits

            #print("Cur len after merging input:", cur_len)
            #print("Outputs logits:", outputs.logits.shape)
            #print("Outputs audio logits:", outputs.audio_logits.shape if outputs.audio_logits is not None else None)
            ...
            # Core CFG and Sampling Logic
            # ==================================
            if is_audio_generation_mode:
                # --- Audio Generation with CFG ---
            ...
            #print("Next audio tokens:", next_audio_tokens if next_audio_tokens is not None else None)
    ...
    def generate(
        ...
        **kwargs,
        ...
        #assert input_ids.shape[0] // (2 if guidance_scale != 1.0 else 1) == 1, (
        ...
        generation_config.generation_kwargs["guidance_scale"] = guidance_scale
        generation_config.generation_kwargs["guidance_scale_text"] = guidance_scale_text
    ...
    @torch.inference_mode()
    def capture_model(self, past_key_values: list[Union[Cache, List[torch.FloatTensor]]], max_batch_size: int) -> None:
