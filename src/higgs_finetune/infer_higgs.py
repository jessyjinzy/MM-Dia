...
from train_higgs import speaker_norm
from evaluation import finer_filter
import json
...
@click.option(
    ...
    type=int,
    default=6,
    help="The maximum rounds of new tokens to generate.",
)
@click.option(
    "--temperature",
    type=float,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Whether to ablate the instruction in the system message.",
    show_default=True,
)
@click.option(
    "--instruct_concat",
    is_flag=True,
    default=False,
    help="Whether to concatenate the instruction with the dialog history.",
    show_default=True,
)
@click.option(
    "--chunk_method",
...
def main(
    ...
    try:
        ...
        try:
            ...
            test_dataset = json.load(open(test_data_path, "r", encoding="utf-8"))

    if do_eval:
        out_path = out_path + '_eval'
        max_rounds = None  # For evaluation, we will use the full dialog 

    if max_rounds is not None:
        out_path = out_path + '_{}_rounds'.format(max_rounds)

    if do_greedy:
        out_path = out_path + '_greedy'

    if instruct:
        out_path = out_path + '_instruct'

    if instruct_tag:
        out_path = out_path + '_instruct_tag'
    
    if instruct_ablation:
        out_path = out_path + '_instruct_ablation'
    
    if instruct_concat:
        out_path = out_path + '_instruct_concat'

    if guidance_scale != 1.0 or guidance_scale_text != 1.0:
        out_path = out_path + '_padleft' if pad_left else out_path + '_padright'
        out_path = out_path + '_gs{:.1f}_text_gs{:.1f}_cfg_style'.format(guidance_scale_text, guidance_scale)
        if cfg_text:
            out_path = out_path + '_cfg_text'
    #if contin:
    #    out_path = out_path + '_continue'

    os.makedirs(out_path, exist_ok=True)

    if do_eval:
        test_dataset = finer_filter(test_dataset)
    else:
        test_dataset = test_dataset.select(range(10))


    speaker_map = [f'[SPEAKER{i}]' for i in range(10)]

    tsv_file = os.path.join(out_path, "_gt_trans.tsv")
    tsv_lines = []
    ...
    for row in tqdm.tqdm(test_dataset):
        ...
        with torch.no_grad():
            ...
            def raw_description_loader(row):
                text = ''
                for i, utt in enumerate(row["srt"]):
                    text += "[{}]{}".format(utt["speaker"], utt["text"])
                text, speaker_list = speaker_norm(text, speaker_map)
                try:
                    orig_speaker_list = row.get("speakers", [])
                    annotation_speaker =  "SPEAKER_MAP: " + "\n".join([
                        "{} -> {}".format(speaker_map[i][1:-1], orig_speaker_list[int(speaker)].strip().replace(':', ''))
                            for i, speaker in enumerate(speaker_list)])
                except:
                    annotation_speaker = ''

                tsv_lines.append("{}\t{}".format(row["clip_id"], text.replace('\n', ' ')))
                
                instruction = annotation_speaker + '\n' + row["style_annotation"]["Summary"]
                output_path = os.path.join(out_path, "output_{}.wav".format(row["clip_id"]))
                return text, instruction, output_path

            # this is for out-of-domain raw tag input
            def raw_tag_loader(row):
                text = ''
                for i, utt in enumerate(row["srt"]):
                    text += "[{}]{}".format(utt["speaker"], utt["text"])
                text, speaker_list = speaker_norm(text, speaker_map)
                ...
                annotation_tag = row["style_annotation"]["Tags"]
                instruction = 'Relationship: {}, Interaction Type: {}, Emotional State: {}'.format(annotation_tag[0], annotation_tag[1], annotation_tag[2])
                output_path = os.path.join(out_path, "output_{}.wav".format(row["clip_id"]))
                return text, instruction, output_path

            # this is for intra-domain test set
            def normal_loader(row):
                text = ''
                if max_rounds is not None:
                    row["segments"] = row["segments"][:max_rounds]
                for segment in row["segments"]:
                    #print("Processing segment:", segment["segment_id"], "Speaker:", segment["speaker"], "Text:", segment["text"])
                ...
                tsv_lines.append("{}\t{}".format(row["dialog_id"], text.replace('\n', ' ')))
                ...
                elif isinstance(annotation_text, list):
                    annotation_text = random.choice(annotation_text)
                else:
                    annotation_text = ' '

                annotation_text = annotation_speaker + '\n' + annotation_text

                #print(text.replace('[', '\n['))
                #print(f"Annotation tag: {annotation_tag}")
                #print(f"Annotation text: {annotation_text}")

                if instruct_tag:
                    instruction = annotation_tag 
                elif instruct:
                    instruction = annotation_text
                elif instruct_ablation:
                    instruction = ' '
                elif instruct_concat:
                    instruction = annotation_tag + '.\n' + annotation_text

                output_path = os.path.join(out_path, "output_{}.wav".format(row["dialog_id"]))
                return text, instruction, output_path

            text, instruction, output_path = normal_loader(row) 
            #text, instruction, output_path = raw_description_loader(row)
            #text, instruction, output_path = raw_tag_loader(row)
            ...
            gt_audio = torch.tensor(row["higgs_codes"]).to(audio_tokenizer.device)

            gt_audio = audio_tokenizer.decode(gt_audio.unsqueeze(0))[0, 0]
            gt_audio_path = os.path.join(out_path, "gt_{}.wav".format(row["dialog_id"]))
            sf.write(gt_audio_path, gt_audio, 24000)
