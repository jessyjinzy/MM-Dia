...
UNCONDITIONAL_SYSTEM_PROMPT = """You are an AI assistant designed to convert text into speech.
...
PREDEFINED_EXAMPLES = {
    ...
    "multispeaker-voice-description": {
        ...
        " or \n"
        "SPEAKER_MAP:SPEAKER_MAP: SPEAKER0 -> Amy\n"
        "SPEAKER1 -> Rachel \n "
...
def text_to_speech(
    ...
    try:
        ...
        unconditional_chatml_sample = prepare_chatml_sample(voice_preset, " " if cfg_text else text, reference_audio, reference_text, UNCONDITIONAL_SYSTEM_PROMPT)
        ...
        logger.info(
            ...
            f"ras_win_len={ras_win_len}, ras_win_max_num_repeat={ras_win_max_num_repeat}, guidance_scale={guidance_scale}, cfg_text={cfg_text}"
        )

        if guidance_scale != 1.0:
            logger.info(f"{request_id}: Using classifier-free guidance with scale {guidance_scale}")
            logger.info(f"{request_id}: Unconditional input: {unconditional_chatml_sample.messages[-1].content[:100]}...")
        ...
        response = engine.generate(
            ...
            unconditional_chat_ml_sample=unconditional_chatml_sample,
        )
...
def create_ui():
    ...
    default_template = "multispeaker-voice-description"
            ...
            with gr.Column(scale=2):
                ...
                with gr.Accordion("Advanced Parameters", open=False):
                    guidance_scale = gr.Slider(
                        ...
                        value=1.5,
                        step=0.5,
                        label="Guidance Scale for Context Prompt",
                    )
                    cfg_text = gr.Checkbox(label="Whether to use empty text for unconditional input in classifier-free guidance", value=True)
...
def main():
    ...
    demo = create_ui()
    demo.launch(server_name=args.host, server_port=args.port, share=True)#, mcp_server=True)
